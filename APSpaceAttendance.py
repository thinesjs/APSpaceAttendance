...
attendix_url = "https://apspace.apu.edu.my/attendix/update"
    ...
    try:
        ...
        try:
            if response['errors'][0]['message'] == "Class not found":
                if attendix_attempt != 3:
                    ...
                    print("Scanning for QR every 3 seconds...")
                    check_qr(capture_window())
                else:
                    print("Attendix: " + response['errors'][0]['message'])
        ...
        try:
            if response['data']['updateAttendance']['attendance'] == "Y":
                ...
                if oncompletion_config == "shutdown":
...
class APSpaceAttendance(object):
    def __init__(self):
        ...
        if oncompletion_config == "shutdown":
            print("[ALERT]")
            print("You have set to shutdown system after updating attendance in config.ini!\nMake sure all important "
                  "applications "
                  "are saved and closed before running this program to avoid file corruptions.")
